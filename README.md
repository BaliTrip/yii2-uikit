...
```
php composer.phar require --prefer-dist balitrip/yii2-uikit
```
or add
...
```php
\balitrip\uikit\UikitAsset::register($this);
```
Uikit Slideshow widget examples:
 * render slideshow from webroot path with images
```php
<?= \balitrip\uikit\widgets\Slideshow::widget(['path' => '/images/somefolder/']) ?>
```
 * render slideshow with custom slides
```php
<?= \balitrip\uikit\widgets\Slideshow::widget(['slides' => [
...
```php
<?= \balitrip\uikit\widgets\Slideshow::widget(['items' => [
